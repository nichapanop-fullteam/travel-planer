# คู่มือทำงานร่วมกัน — PunGuide Prototype
...
>
> **ทุกงานต้องแตก branch ใหม่มาทำก่อนเสมอ** ไม่ว่างานจะเล็กแค่ไหนก็ตาม (แก้ typo, ปรับสีนิดเดียว ก็ต้องแตก branch) ห้าม `git commit` หรือ `git push` ขณะอยู่บน `main` โดยเด็ดขาด
>
> เหตุผล: `main` คือโค้ดกลางที่ทุกคนดึงไปใช้ต่อ ถ้ามีใคร push ตรงเข้าไปแล้วโค้ดพัง (build ไม่ผ่าน, หน้าเว็บ error) **ทุกคนในทีมจะโดนผลกระทบทันที** เพราะดึงโค้ดพังมาต่อยอดกันหมด การแตก branch + เปิด PR คือด่านให้เพื่อนช่วยดูก่อนที่โค้ดจะไปอยู่ใน `main` จริงๆ
>
> ทุกครั้งก่อนพิมพ์ `git commit` ให้เช็คก่อนว่าตัวเองอยู่ branch ไหน:
> ```bash
> git branch --show-current
> ```
> ถ้าขึ้นว่า `main` → **ห้าม commit** ให้ `git checkout -b ชื่อ-branch-ใหม่` ก่อนเสมอ (ดูวิธีตั้งชื่อ branch ในหัวข้อถัดไป)
