...
// not the window — is what moves, so the feed's chrome (tabs, mute, the
// desktop stepper) can sit still on top of it.
//
// Two shapes out of one tree, split at 1025px like the rest of the app
// (MobileBottomNav, FrostedTopNav):
//
// - At or below: the post is the screen. Edge to edge, square corners, and
//   every control overlaid on the media — the immersive treatment the design
//   asks for, with the bottom bar the only chrome left.
// - Above: the post becomes a 9:16 card floating on the page, with a lime
//   up/down stepper beside it, because a full-bleed phone feed stretched
//   across a desktop is neither.
//
...
// 9:16 card still fits a laptop's short screen. 0.5625 is 9/16; the 13.5rem is
// the header rows above the stage plus the band the tabs and the mute button
// need above the card. Lives as a custom property because the tab row and the
// stepper both position themselves against the card's edges, and the card is
// centred — so all three need the same number.
const CARD_WIDTH = "min(360px, calc((100dvh - 13.5rem) * 0.5625))";

export function PuntokFeed({ clips }: { clips: PuntokClip[] }) {
  ...
  // A new tab is a new list, so the old index would point at someone else's
  // post — and switching without resetting leaves the scroller parked on a row
  // that no longer exists. Done here rather than in an effect on `tab`: this is
  // the event that changes the list, and an effect would only re-render again
  // to say the same thing.
  const selectTab = useCallback((next: FeedTab) => {
    setTab(next);
    setActiveIndex(0);
  ...
  return (
    <div
      className="relative h-full overflow-hidden bg-black min-[1025px]:bg-[var(--color-surface)]"
      style={{ ["--puntok-card-w" as string]: CARD_WIDTH }}
    >
      {/* Feed-level chrome, so it lives outside the scroller and stays put
          while posts move under it. On a phone it is overlaid on the clip; on
          desktop it narrows to the card's own width and so ends up in the band
          above it, which is why CARD_WIDTH leaves that band free. */}
      <div
        className="pointer-events-none absolute inset-x-0 top-0 z-20 flex items-start justify-between px-4 pb-4 min-[1025px]:mx-auto min-[1025px]:w-[var(--puntok-card-w)] min-[1025px]:px-0"
      ...
      >
        ...
        <div
          ...
          className="pointer-events-auto flex items-center gap-1 rounded-full bg-black/35 p-1 backdrop-blur-md min-[1025px]:bg-black/10"
        >
          {TABS.map((item) => {
              ...
              <button
                ...
                className={`rounded-full px-3.5 py-1.5 text-xs font-bold transition ${
                  isActive
                    ...
                    : "text-white/85 min-[1025px]:text-[var(--foreground)]/60"
                }`}
              >
                {item.label}
              </button>
        ...
        <button
          ...
          aria-pressed={!muted}
          className="pointer-events-auto flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-black/35 text-white backdrop-blur-md transition hover:bg-black/50 min-[1025px]:bg-black/10 min-[1025px]:text-[var(--foreground)] min-[1025px]:hover:bg-black/20"
        >
      ...
      {visible.length === 0 ? (
        <EmptyFollowing onBrowse={() => selectTab("forYou")} />
      ) : (
        <>
          <div
            ref={scrollerRef}
...
function StepButton({
  ...
  return (
    <button
      ...
      disabled={disabled}
      aria-label={label}
      className="flex h-10 w-10 items-center justify-center rounded-full bg-[var(--color-accent-lime)] text-[var(--foreground)] shadow-[var(--shadow-md)] transition hover:brightness-95 disabled:opacity-35"
    >
...
function ClipCard({
  ...
  return (
    <article className="relative h-full w-full overflow-hidden bg-black min-[1025px]:aspect-[9/16] min-[1025px]:h-auto min-[1025px]:w-[var(--puntok-card-w)] min-[1025px]:rounded-[28px] min-[1025px]:shadow-[0_18px_50px_-12px_rgba(16,24,40,0.45)]">
      {clip.videoUrl ? (
        <video
          ref={videoRef}
          src={clip.videoUrl}
      ...
      <div className="absolute inset-x-0 bottom-0 z-10 flex flex-col gap-2 p-4 pr-16">
        <div className="flex items-center gap-2">
          <span className="text-sm font-bold text-white drop-shadow">{clip.creatorName}</span>
          <button
            type="button"
        ...
        <p className="text-[13px] font-medium leading-snug text-white/95 drop-shadow">
          {clip.caption}
        </p>

        <p className="flex flex-wrap gap-x-2 text-[11px] font-semibold text-white/75">
          {clip.tags.map((tag) => (
...
function ActionRail({
  ...
  return (
    <div className="absolute bottom-4 right-2.5 z-10 flex flex-col items-center gap-4">
      <div className="relative pb-1.5">
        ...
        {!isFollowed && (
          <button
            ...
            className="absolute -bottom-0.5 left-1/2 flex h-5 w-5 -translate-x-1/2 items-center justify-center rounded-full bg-[var(--color-accent-orange)] text-white ring-2 ring-black/10 transition hover:brightness-105"
          >
      ...
      >
        <Bookmark
          size={25}
          strokeWidth={1.8}
          className={isSaved ? "text-[var(--color-accent-lime)]" : ""}
      ...
      {clip.tripHref && (
        <Link
          ...
          className="h-10 w-10 overflow-hidden rounded-xl border-2 border-white/80"
        >
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={clip.imageUrl} alt="" className="h-full w-full object-cover" />
        </Link>
...
function RailButton({
  ...
  return (
    <button
      ...
      disabled={!onClick}
      className="flex flex-col items-center gap-1 text-white drop-shadow-[0_1px_3px_rgba(0,0,0,0.5)] transition active:scale-90 disabled:active:scale-100"
    >
      {children}
